...
**Цель:** главное окно KaminIDE на GPUI (нативный Rust, стек Zed) — без WebView2 в главном окне (уходит наблюдавшийся фриз из-за защитного ПО: виснет именно главный WebView2, встроенные вебвью-панели не виснут). VSIX-вебвью (чат Бриджа и пр.) — через gpui-wry (WebView2-оверлей, не фризит). Экстеншн-хост (node, kamin-host) сохраняется без изменений.
