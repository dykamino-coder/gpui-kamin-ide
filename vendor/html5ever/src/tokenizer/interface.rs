...
// KaminIDE patch: инструкция обработки наравне с прочими токенами.
pub use self::Token::ProcessingInstructionToken;
...
pub enum Token {
    ...
    /// KaminIDE patch: инструкция обработки `<?target data>`.
    ///
    /// Разметка HTML их не описывает, но браузеры кладут в дерево именно
    /// узел инструкции, а не комментарий: `document.body.childNodes[0]` у
    /// Chrome на `<body><?something>` — узел типа 7. Набор WPT это и
    /// проверяет (`processing-instructions.dat`), а прежний разбор делал
    /// «мусорный комментарий» и расходился со всеми браузерами.
    ProcessingInstructionToken(StrTendril, StrTendril),
