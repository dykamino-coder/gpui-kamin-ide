...
// KaminIDE patch: инструкции обработки (`<?…>`).
pub use self::interface::ProcessingInstructionToken;
...
pub struct Tokenizer<Sink> {
    ...
    /// KaminIDE patch: имя и данные инструкции обработки (`<?target data>`).
    current_pi_target: RefCell<StrTendril>,
    current_pi_data: RefCell<StrTendril>,
    /// Сырой текст инструкции: негодное имя превращает её в комментарий, и
    /// тогда в дерево уходит именно то, что стояло в разметке.
    current_pi_raw: RefCell<StrTendril>,
...
impl<Sink: TokenSink> Tokenizer<Sink> {
    ...
    pub fn new(sink: Sink, mut opts: TokenizerOpts) -> Tokenizer<Sink> {
        ...
        Tokenizer {
            ...
            current_pi_raw: RefCell::new(StrTendril::new()),
    ...
    /// KaminIDE patch: годится ли имя инструкции обработки.
    ///
    /// Правила имени XML: первым идёт буква, `_` или `:`, дальше к ним
    /// добавляются цифры, точка и дефис. Негодное имя (`<?a$>`) браузер
    /// кладёт в дерево комментарием, а не инструкцией.
    fn pi_target_is_name(target: &str) -> bool {
    ...
    /// KaminIDE patch: становится ли запись комментарием, а не инструкцией.
    ///
    /// Негодное имя (`<?a$>`, `<?data.v1>`) и имена, начинающиеся на `xml`
    /// в любом регистре (их резервирует разметка XML), браузер кладёт в
    /// дерево комментарием.
    fn pi_is_comment(target: &str) -> bool {
        !Self::pi_target_is_name(target)
            || target
                .get(..3)
                .is_some_and(|p| p.eq_ignore_ascii_case("xml"))
    }

    /// KaminIDE patch: отдать инструкцию обработки построителю дерева.
    ///
    /// Хвостовой `?` перед `>` — часть закрытия, в данные он не идёт.
    /// Инструкция с негодным именем уходит комментарием, безымянная (`<?`
    /// и сразу конец) — не уходит вовсе.
    fn emit_current_pi(&self) {
        let target = mem::take(&mut *self.current_pi_target.borrow_mut());
        let mut data = mem::take(&mut *self.current_pi_data.borrow_mut());
        let raw = mem::take(&mut *self.current_pi_raw.borrow_mut());
        if Self::pi_is_comment(&target) {
            if raw.len() > 1 {
                self.process_token_and_continue(CommentToken(raw));
            }
            return;
        }
        if data.ends_with('?') {
        ...
        self.process_token_and_continue(ProcessingInstructionToken(target, data));
    }
...
macro_rules! shorthand (
    ...
    ( $me:ident : clear_pi                         ) => ({ $me.current_pi_target.borrow_mut().clear();
                                                           $me.current_pi_data.borrow_mut().clear();
                                                           $me.current_pi_raw.borrow_mut().clear();
                                                           $me.current_pi_raw.borrow_mut().push_char('?');    });
    ( $me:ident : push_pi_target $c:expr           ) => ({ $me.current_pi_target.borrow_mut().push_char($c);
                                                           $me.current_pi_raw.borrow_mut().push_char($c);     });
    ( $me:ident : push_pi_raw $c:expr              ) => ( $me.current_pi_raw.borrow_mut().push_char($c)   );
...
impl<Sink: TokenSink> Tokenizer<Sink> {
        ...
        match self.state.get() {
            ...
            states::TagOpen => loop {
                match get_char!(self, input) {
                    ...
                    '?' => {
                        // KaminIDE patch: инструкция обработки, а не мусорный
                        // комментарий — см. `ProcessingInstructionToken`.
                        self.bad_char_error();
                        go!(self: clear_pi; to PiTarget)
                    }
            ...
            // KaminIDE patch: имя инструкции обработки — до пробела или `>`.
            states::PiTarget => loop {
                match get_char!(self, input) {
                    '>' => go!(self: emit_pi; to Data),
                    '\0' => go!(self: push_pi_target '\u{fffd}'),
                    // Вопросительный знак имя заканчивает, но сам идёт в
                    // данные: `<?hey?there>` — имя `hey` и данные
                    // `?there`, а `<?good?>` — имя `good` без данных.
                    '?' => go!(self: reconsume PiData),
                    c if c.is_ascii_whitespace() => go!(self: push_pi_raw c; to PiDataStart),
                    c => go!(self: push_pi_target c),
                }
            },

            // KaminIDE patch: пробелы между именем и данными не значимы.
            states::PiDataStart => loop {
                match get_char!(self, input) {
                    '>' => go!(self: emit_pi; to Data),
                    c if c.is_ascii_whitespace() => go!(self: push_pi_raw c),
                    _ => go!(self: reconsume PiData),
                }
            },

            // KaminIDE patch: данные инструкции обработки — до `>`.
            states::PiData => loop {
                match get_char!(self, input) {
                    '>' => go!(self: emit_pi; to Data),
                    '\0' => go!(self: push_pi_data '\u{fffd}'),
    ...
    fn eof_step(&self) -> ProcessResult<Sink::Handle> {
        ...
        match self.state.get() {
            ...
            // KaminIDE patch: инструкция без `>` до конца ввода отбрасывается.
            // KaminIDE patch: незакрытая инструкция на конце ввода уходит
            // комментарием — так же поступает браузер; пустая (`<?`) исчезает.
            states::PiTarget | states::PiDataStart | states::PiData => {
                let raw = mem::take(&mut *self.current_pi_raw.borrow_mut());
                let target = mem::take(&mut *self.current_pi_target.borrow_mut());
                self.current_pi_data.borrow_mut().clear();
                // Запись с годным именем без `>` теряется целиком: она ещё не
                // инструкция. Негодная давно стала комментарием, и комментарий
                // на конце ввода отдаётся как есть.
                if raw.len() > 1 && Self::pi_is_comment(&target) {
                    self.process_token_and_continue(CommentToken(raw));
                }
                go!(self: to Data)
            }
