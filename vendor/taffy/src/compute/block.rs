...
fn perform_final_layout_on_in_flow_children(
    ...
    for item in items.iter_mut() {
        if item.position == Position::Absolute {
            ...
            let item_first_baseline = item_layout.first_baselines.y;
            ...
            // KaminIDE patch: блочный контейнер ПРОПУСКАЕТ вверх базовую линию
            // первого потокового ребёнка. Без этого абзац, завёрнутый в
            // обычный `div`, для гибкой раскладки базовой линии не имеет, и
            // `align-items: baseline` выравнивает по нижним краям коробок.
