...
impl Render for SearchPanel {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            // KaminIDE patch: `items_center` в КОЛОНКЕ центрирует по ширине —
            // строка бара переставала растягиваться, и поле ввода схлопывалось;
            // `overflow_hidden` — узкий редактор иначе выносит кнопки за бар
            .overflow_hidden()
            .py_2()
            .px_3()
            .w_full()
            ...
            .child(
                h_flex()
                    ...
                    .gap_2()
                    .child(
                        // KaminIDE patch: `flex_1` без `w_full` схлопывал поле
                        // ввода до ширины суффикса «Aa» — искать было негде
                        div()
                            .flex_1()
                            .w_full()
                            // ширина Monaco-виджета поиска: без минимума
                            // `flex_1` схлопывает поле до суффикса «Aa»
                            .min_w(gpui::px(120.))
                            .gap_1()
                            .child(
                    ...
                    .child(
                        Label::new(self.matcher.label())
                            ...
                            .text_left()
                            // счётчик «0/0» и пустой распорка `w_7` съедали
                            // 92px там, где редактор бывает уже 300px
                            .min_w_10(),
                    )
                    .child(
                        Button::new("close")
