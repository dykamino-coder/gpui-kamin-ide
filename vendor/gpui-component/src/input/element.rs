...
impl TextElement {
    ...
    fn layout_lines(
        ...
        let mut lines = vec![];
        let mut offset = 0;
        // KaminIDE patch (#76): runs_for_range сканировал `runs` с нуля на
        // КАЖДУЮ wrapped-строку — O(строки × runs) на prepaint (у больших
        // вьюпортов это тысячи повторных проходов за кадр). Диапазоны идут
        // строго по возрастанию, поэтому держим внешний курсор и двигаем его
        // только вперёд: суммарно O(runs) на кадр.
        ...
        for (ix, line) in visible_text.split("\n").enumerate() {
            ...
            for range in &line_item.wrapped_lines {
                ...
                while run_ix < runs.len() && run_start + runs[run_ix].len <= abs.start {
                    run_start += runs[run_ix].len;
                    run_ix += 1;
                }
                // Граничный run может перетекать в следующий диапазон —
                // локальный проход не двигает внешний курсор.
                let mut line_runs: Vec<TextRun> = Vec::new();
                let (mut jx, mut cursor) = (run_ix, run_start);
                while jx < runs.len() && cursor < abs.end {
                    let run = &runs[jx];
                    let s = abs.start.max(cursor);
                    let e = abs.end.min(cursor + run.len);
                    if e > s {
                        line_runs.push(TextRun { len: e - s, ..run.clone() });
                    }
                    cursor += run.len;
                    jx += 1;
                }
    ...
    fn highlight_lines(
        ...
        // KaminIDE patch (#76): спан профиля — где именно жир TextElement.
        let _prof = gpui::prepaint_prof::enter("highlight_lines");
        let state = self.state.read(cx);
        let text = &state.text;
        ...
        // KaminIDE patch (#76): склейка per-line стилей неизменна, пока не
        // менялись текст (update() чистит кэш) и тема — на стабильном
        // вьюпорте (минимап, повторные кадры) весь цикл заменяется одним
        // лукапом. При скролле ключ промахивается, но per-line записи внутри
        // styles() продолжают попадать — хуже не становится.
        let theme = &cx.theme().highlight_theme;
        if let Some(hit) = highlighter.cached_styles(&visible_byte_range, theme) {
            styles = hit;
        } else {
            for line in text
                .iter_lines()
                ...
                .take(visible_range.len())
            {
                ...
                // KaminIDE patch (#76): диапазоны строк упорядочены и НЕ
                // пересекаются — append эквивалентен combine, а
                // combine_highlights(styles, …).collect() на каждой строке
                // пересобирал растущий вектор: O(n²), ~3.6 мс на prepaint-вызов
                // (90% жира TextElement по спан-профилю).
                styles.extend(line_styles);

                offset = range.end;
            }
            highlighter.store_styles(&visible_byte_range, theme, &styles);
        }
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (line_number_width, line_number_len) = {
            // KaminIDE patch (#76): спан профиля.
            let _prof = gpui::prepaint_prof::enter("layout_line_numbers");
        ...
        let lines = {
            // KaminIDE patch (#76): спан профиля.
            let _prof = gpui::prepaint_prof::enter("layout_lines");
            Self::layout_lines(
                &state,
                &display_text,
                &last_layout,
    ...
    fn paint(
        ...
        // KaminIDE patch: минимапа — зеркало без ввода (Zed EditorMode::Minimap:
        // read_only, без подписок, без каретки).
        let is_minimap = self.state.read(cx).minimap;
        ...
        // Paint selections
        if window.is_window_active() && !is_minimap {
        ...
        if focused && show_cursor && !is_minimap {
...
// KaminIDE patch (#76): в layout_lines заменён инкрементальным курсором;
// остаётся эталоном для тестов курсора.
#[allow(unused)]
