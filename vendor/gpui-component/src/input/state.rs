...
pub struct InputState {
    ...
    /// KaminIDE patch: режим минимапы — аналог `EditorMode::Minimap` в Zed.
    /// Зеркало текста, которое НЕ участвует во вводе: не ставит оконные
    /// mouse-листенеры и IME-хендлер, не рисует выделение и каретку, и берёт
    ...
    pub scroll_handle: ScrollHandle, // KaminIDE patch: sticky-scroll needs the offset
...
impl InputState {
    ...
    /// KaminIDE patch: включить режим минимапы (зеркало текста без ввода).
    ///
    /// Порт `EditorMode::Minimap` из Zed: сам текст рисуется штатным layout-ом,
    /// но состояние не участвует в фокусе и мыши, а высота строки приходит от
    /// родителя (`.line_height(...)` на обёртке).
    ...
    /// KaminIDE patch (минимапа): total_lines + line_height последнего layout.
    /// KaminIDE patch (минимапа/скроллбар): высота прокручиваемого контента —
    /// та же величина, по которой редактор ограничивает СВОЮ прокрутку. Она
    /// включает нижний запас (`BOTTOM_MARGIN_ROWS` = 3 строки в высоком вью,
    /// иначе одна), поэтому считать предел как `total − visible` нельзя: палка
    /// упиралась в низ раньше, чем колесо доезжало до конца.
    pub fn scroll_content_height(&self) -> Pixels {
        self.scroll_size.height
    }

    pub fn minimap_metrics(&self) -> (usize, Option<Pixels>) {
        (
            self.text.len_lines(ropey::LineType::LF),
            self.last_layout.as_ref().map(|l| l.line_height),
        )
    }

    /// KaminIDE patch (минимапа): (indent_chars, len_chars) строки row —
    /// силуэт текста без шейпинга. Скан капнут 400 символами.
    pub fn minimap_line(&self, row: usize) -> Option<(usize, usize)> {
        if row >= self.text.len_lines(ropey::LineType::LF) {
            return None;
        }
        let line = self.text.line(row, ropey::LineType::LF);
    ...
    /// KaminIDE patch (минимапа, глифы): текст строки row, cap 160 симв.
    pub fn minimap_line_text(&self, row: usize) -> Option<String> {
        if row >= self.text.len_lines(ropey::LineType::LF) {
            return None;
        }
        let line = self.text.line(row, ropey::LineType::LF);
