...
pub struct Input {
    ...
    bordered: bool,
    focus_bordered: bool,
    /// KaminIDE patch: не рисовать ВСТРОЕННЫЙ скроллбар (у редактора свой,
    /// правее глиф-бара — иначе видно две полосы).
    hide_scrollbar: bool,
    tab_index: isize,
    selected: bool,
}
...
impl Input {
    /// KaminIDE patch: скрыть встроенный скроллбар (свой рисуем сами).
    ...
    fn render_editor(
        ...
        v_flex()
            ...
            .child(div().flex_1().child(input_state.clone()).map(|this| {
                if let Some(last_layout) = state.last_layout.as_ref().filter(|_| !hide_scrollbar) {
...
impl RenderOnce for Input {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let hide_scrollbar = self.hide_scrollbar;
        ...
        div()
            ...
            // KaminIDE patch: в режиме минимапы зеркало не трогает мышь —
            // ни выделения, ни своей прокрутки (колесо уходит контейнеру
            // минимапы, который двигает ОСНОВНОЙ редактор).
            .when(!state.minimap, |this| {
                this.on_mouse_down(
                    MouseButton::Left,
            ...
            .size_full()
            // Zed берёт line-height минимапы из ТОГО ЖЕ text_style с
            // подменённым font_size (`get_minimap_line_height`), поэтому здесь
            // жёсткий Rems(1.25) обязан отступить и высота строки приходит от
            // родителя.
            .when(!state.minimap, |this| this.line_height(LINE_HEIGHT))
            .input_px(self.size)
            .input_py(self.size)
            .input_h(self.size)
            .input_text_size(self.size)
            // KaminIDE patch: над минимапой курсор обычный — это не поле ввода
            .when(!state.minimap, |this| this.cursor_text())
            .items_center()
            .when(state.mode.is_multi_line(), |this| {
                this.h_auto()
