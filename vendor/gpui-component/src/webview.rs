...
///
/// [experimental]
/// KaminIDE patch: freeze ресайза wv2-чайлдов на время драга сплиттеров.
static WV_FREEZE: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);

pub fn set_webview_bounds_frozen(v: bool) {
    WV_FREEZE.store(v, std::sync::atomic::Ordering::Relaxed);
}

pub fn webview_bounds_frozen() -> bool {
    WV_FREEZE.load(std::sync::atomic::Ordering::Relaxed)
}
...
impl Element for WebViewElement {
    ...
    fn prepaint(
        ...
        // KaminIDE patch: bounds must be pushed even while hidden — a child
        // shown later otherwise keeps stale (zero) bounds until the next
        // layout change (symptom: empty panel until the user resizes).
        let visible = self.parent.read(cx).visible();

        // KaminIDE patch: во время драга сплиттеров ресайз wv2 заморожен
        // (внутренний рендер WebView2 асинхронен → дёргалось «сдвиг, потом
        // расширение»); хост-приложение обрезает окно регионом и делает
        // один финальный ресайз на отпускании.
        if webview_bounds_frozen() {
            return if visible {
