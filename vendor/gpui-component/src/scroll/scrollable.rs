...
pub trait ScrollableElement: InteractiveElement + Styled + ParentElement + Element {
    ...
    /// KaminIDE patch: как `overflow_y_scrollbar`, но с ВНЕШНИМ ScrollHandle —
    /// хост может программно скроллить (scroll-to-row в дереве).
    #[track_caller]
    fn overflow_y_scrollbar_with(self, handle: &ScrollHandle) -> Scrollable<Self> {
        let mut s = Scrollable::new(self, ScrollbarAxis::Vertical);
        s.external = Some(handle.clone());
        s
    }
...
pub struct Scrollable<E: InteractiveElement + Styled + ParentElement + Element> {
    ...
    element: E,
    axis: ScrollbarAxis,
    external: Option<ScrollHandle>,
    /// KaminIDE patch: сдвиг вертикального бара от правого края, px.
    bar_inset_right: f32,
}
    ...
    E: InteractiveElement + Styled + ParentElement + Element,
{
    ...
    /// KaminIDE patch: отодвинуть вертикальный скроллбар от правого края
    /// (зазор бар↔кромка панели).
    pub fn scrollbar_inset_right(mut self, inset: f32) -> Self {
        self.bar_inset_right = inset;
        self
    }
}
    ...
    E: InteractiveElement + Styled + ParentElement + Element + 'static,
{
    fn render(mut self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        let scroll_handle = self.external.take().unwrap_or_else(|| {
            window
        ...
        *self.element.style() = StyleRefinement::default();
        ...
        div()
            ...
            .child(
                // KaminIDE patch: inset сдвигает правую грань слоя бара —
                // Scrollbar якорит тамб к ней.
                render_scrollbar("scrollbar", &scroll_handle, self.axis, window, cx)
                    .right(gpui::px(self.bar_inset_right)),
            )
    }
}
