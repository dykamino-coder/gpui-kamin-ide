...
pub trait Styled: Sized {
    ...
    /// KaminIDE patch: трекинг (`letter-spacing`). Работает у ОДНОСТРОЧНОГО
    /// текста без переноса — см. `layout_wrapped_line_spaced` (план 99).
    fn letter_spacing(mut self, spacing: impl Into<crate::Pixels>) -> Self {
        self.text_style()
            .get_or_insert_with(Default::default)
