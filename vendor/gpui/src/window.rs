...
pub struct Window {
    ...
    pub(crate) focus: Option<FocusId>,
    /// KaminIDE patch: фокус пришёл с КЛАВИАТУРЫ (`:focus-visible` браузера).
                        ...
                        handle
                            .update(&mut cx, |_, window, cx| {
                                ...
                                window.present();
                                {
                                    ...
                                    DRAW_US.fetch_add((t1 - t0).as_micros() as u32, Relaxed);
                                    PRESENT_US.fetch_add(t1.elapsed().as_micros() as u32, Relaxed);
...
impl Window {
    ...
    /// KaminIDE patch: фокус получен клавиатурой — рисуем `:focus-visible`.
    ...
    /// KaminIDE patch: raw ID3D11Device окна — по нему открывается общий
    /// handle текстуры от CEF (Windows; на прочих платформах None).
    pub fn d3d_device_raw(&self) -> Option<*mut std::ffi::c_void> {
        self.platform_window.d3d_device_raw()
    }

    /// KaminIDE patch: raw ID3D11DeviceContext окна.
    pub fn d3d_context_raw(&self) -> Option<*mut std::ffi::c_void> {
        self.platform_window.d3d_context_raw()
    }

    /// KaminIDE patch: принять ЧУЖУЮ текстуру D3D11 (готовый кадр браузера) и
    /// получить её описатель для отрисовки. Указатель должен быть с AddRef.
    pub fn register_external_texture(
        &self,
        texture: *mut std::ffi::c_void,
        width: i32,
        height: i32,
    ) -> Option<ExternalTexture> {
        let size = size(DevicePixels(width), DevicePixels(height));
        self.platform_window
            .register_external_texture(texture, size)
            .map(ExternalTexture)
    }

    /// KaminIDE patch: подменить кадр в УЖЕ выданном описателе: запись
    /// атласа та же, меняется только её содержимое. `size` — новый размер
    /// кадра в физических px.
    pub fn update_external_texture(
        ...
        if !self
            .platform_window
            .update_external_texture(texture.0.texture_id, raw)
        {
            return None;
        }
        Some(texture.with_size(width, height))
    }

    /// KaminIDE patch: забыть чужую текстуру.
    pub fn drop_external_texture(&self, texture: &ExternalTexture) {
        self.platform_window
            .unregister_external_texture(texture.0.texture_id);
    }

    /// KaminIDE patch: нарисовать чужую текстуру как обычный спрайт — с теми
    /// же скруглением и маской, что у картинок.
    ///
    /// Зовётся в фазе отрисовки элемента.
    ...
    fn draw_roots(&mut self, cx: &mut App) {
        ...
        let _hit_started = std::time::Instant::now();
        self.mouse_hit_test = self.next_frame.hit_test(self.mouse_position);
        crate::frame_perf::HITTEST_US.fetch_add(
            _hit_started.elapsed().as_micros() as u32,
            std::sync::atomic::Ordering::Relaxed,
        );

        // Now actually paint the elements.
        self.invalidator.set_phase(DrawPhase::Paint);
        let _paint_started = std::time::Instant::now();
        root_element.paint(self, cx);
        crate::frame_perf::PAINT_US.fetch_add(
            _paint_started.elapsed().as_micros() as u32,
            std::sync::atomic::Ordering::Relaxed,
        );
    ...
    ///
    /// KaminIDE patch: размытие подложки под областью (backdrop blur, как
    /// CSS `backdrop-filter: blur`). Рисует в бэкбуфер уже отрисованное ПОД
    /// текущим z-порядком содержимое, размытое каскадом даунсемплов, с маской
    /// скруглённых углов. Windows-only: Surface-примитив здесь свободен.
    ///
    /// This method should only be called as part of the paint phase of element drawing.
    ...
    fn dispatch_mouse_event(&mut self, event: &dyn Any, cx: &mut App) {
        // KaminIDE patch: нажатие мышью гасит `:focus-visible` — как в браузере,
        // где клик даёт `:focus`, но НЕ `:focus-visible`.
        if self.focus_visible && event.downcast_ref::<crate::MouseDownEvent>().is_some() {
