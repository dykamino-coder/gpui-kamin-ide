...
impl TaffyLayoutEngine {
    ...
    pub fn request_layout(
        ...
        crate::frame_perf::LAYOUT_NODES.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn compute_layout(
        ...
        let _taffy_started = std::time::Instant::now();
        self.taffy
            ...
            .expect(EXPECT_MESSAGE);
        crate::frame_perf::TAFFY_US.fetch_add(
...
/// KaminIDE patch: копит время замеров содержимого (`frame_perf::MEASURE_US`).
struct MeasureTimer(std::time::Instant);
impl Drop for MeasureTimer {
    fn drop(&mut self) {
        crate::frame_perf::MEASURE_US.fetch_add(
