...
pub(crate) trait PlatformWindow: HasWindowHandle + HasDisplayHandle {
    ...
    /// KaminIDE patch: raw ID3D11Device окна (Windows), иначе None.
    fn d3d_device_raw(&self) -> Option<*mut std::ffi::c_void> {
        None
    }

    /// KaminIDE patch: raw ID3D11DeviceContext окна (Windows), иначе None.
    fn d3d_context_raw(&self) -> Option<*mut std::ffi::c_void> {
        None
    }

    /// KaminIDE patch: положить чужую текстуру D3D11 в атлас, получить тайл.
    ...
    fn update_external_texture(&self, _id: AtlasTextureId, _texture: *mut std::ffi::c_void) -> bool {
        false
    }

    /// KaminIDE patch: забыть чужую текстуру.
    fn unregister_external_texture(&self, _id: AtlasTextureId) {}
...
impl Image {
    ...
    pub fn to_image_data(&self, svg_renderer: SvgRenderer) -> Result<Arc<RenderImage>> {
        ...
        let frames = match self.format {
            ...
            ImageFormat::Svg => {
                // KaminIDE patch: SVG-ветка (в отличие от всех остальных и от
                // ветки `ImageSource::Resource`) не переводила RGBA→BGRA и не
                // снимала premultiply — картинка из байтов выходила с
                // переставленными R и B. Заодно рендерим в том же
                // SMOOTH_SVG_SCALE_FACTOR, что и ресурсная ветка, иначе иконка
                // мылится.
                let pixmap = svg_renderer.render_pixmap(
                    &self.bytes,
                    SvgSize::ScaleFactor(crate::SMOOTH_SVG_SCALE_FACTOR),
                )?;
                ...
                image.scale_factor = crate::SMOOTH_SVG_SCALE_FACTOR;
