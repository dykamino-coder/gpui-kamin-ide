    ...
    pub fn load(
        ...
        window
            .spawn(cx, {
                async move |cx| {
                    _ = task.await;
                    // KaminIDE patch: notify СРАЗУ, не on_next_frame. Тот ждёт
                    // следующего кадра ОКНА — при точечной перерисовке (кадры
                    // только по заказу) кадра в покое нет, и завершившаяся
