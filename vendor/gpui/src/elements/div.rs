...
impl Interactivity {
    ...
    pub fn paint(
        ...
        window.with_optional_element_state::<InteractiveElementState, _>(
            ...
            |element_state, window| {
                ...
                // KaminIDE patch: запоминаем bounds для hover-рефайна на
                // СЛЕДУЮЩЕМ layout (там hitbox'а нет, а цвет текста
                // запекается именно там)
                    ...
                    && let Some(state) = element_state.as_mut()
                {
                    *state
                        ...
                        .borrow_mut() = hitbox.map(|h| h.bounds);
                }
    ...
    /// KaminIDE patch: `StyleRefinement.text` НЕ помечен `#[refineable]`, и
    /// `Style::refine` заменяет его ЦЕЛИКОМ. Из-за этого `hover`, задающий
    /// один `text_color`, стирал `font_size`/`font_weight` базы — подпись под
    /// курсором прыгала в размере (баг найден юзером на табах стрипа).
    /// Здесь сливаем текстовый под-рефайнмент поле-в-поле.
    fn refine_merge_text(style: &mut Style, refinement: &StyleRefinement) {
        if refinement.text.is_none() {
            style.refine(refinement);
            return;
        }
        let base_text = style.text.clone();
        style.refine(refinement);
        let mut merged = base_text;
        merged.refine(&style.text.clone());
        style.text = merged;
    }
    ...
    fn compute_style_internal(
        ...
        if let Some(focus_handle) = self.tracked_focus_handle.as_ref() {
            ...
            {
                Self::refine_merge_text(&mut style, in_focus_style);
            }

            if let Some(focus_style) = self.focus_style.as_ref()
                && focus_handle.is_focused(window)
            {
                Self::refine_merge_text(&mut style, focus_style);
            }
        }

        // KaminIDE patch: на layout hitbox'а ещё нет, но цвет ТЕКСТА
        // запекается именно здесь. Решаем «наведено ли» по bounds прошлого
        // кадра и позиции курсора — иначе `hover`/`group_hover` красят фон,
        // но не подписи и не кодиконы.
        if hitbox.is_none() && !cx.has_active_drag() {
            let mouse = window.mouse_position();
            if let Some(group_hover) = self.group_hover_style.as_ref()
                && GroupBounds::get(&group_hover.group, cx)
                    .is_some_and(|bounds| bounds.contains(&mouse))
            {
                Self::refine_merge_text(&mut style, &group_hover.style);
            }
            if let Some(hover_style) = self.hover_style.as_ref()
                ...
                && state
                    ...
                    .is_some_and(|bounds| bounds.contains(&mouse))
            {
                Self::refine_merge_text(&mut style, hover_style);
            }
        }
                ...
                if can_drop {
                    for (state_type, group_drag_style) in &self.group_drag_over_styles {
                        if let Some(group_hitbox_id) =
                            ...
                            && group_hitbox_id.is_hovered(window)
                        {
                            Self::refine_merge_text(&mut style, &group_drag_style.style);
                        }
                    }
                    ...
                    for (state_type, build_drag_over_style) in &self.drag_over_styles {
                        ...
                        {
                            Self::refine_merge_text(
                                &mut style,
                                &build_drag_over_style(drag.value.as_ref(), window, cx),
                            );
                        }
                    }
                }
        ...
        if let Some(element_state) = element_state {
            ...
            {
                Self::refine_merge_text(&mut style, &group.style)
            }

            if let Some(active_style) = self.active_style.as_ref()
                && clicked_state.element
            {
                Self::refine_merge_text(&mut style, active_style)
            }
        }
...
pub struct InteractiveElementState {
    /// KaminIDE patch: bounds последнего paint — по ним на layout решаем,
    /// наведён ли элемент (см. `compute_style_internal`).
    pub(crate) last_bounds: Option<Rc<RefCell<Option<Bounds<Pixels>>>>>,
...
/// KaminIDE patch: bounds групп с последнего paint — `group_hover` должен
/// работать и на layout (там hitbox'ов ещё нет).
#[derive(Default)]
...
impl GroupBounds {
    pub fn get(name: &SharedString, cx: &mut App) -> Option<Bounds<Pixels>> {
    ...
    pub fn set(name: SharedString, bounds: Bounds<Pixels>, cx: &mut App) {
        let stack = cx.default_global::<Self>().0.entry(name).or_default();
        stack.clear();
        stack.push(bounds);
    }
}
