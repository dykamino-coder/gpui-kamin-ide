...
impl LineLayoutCache {
    ...
    pub fn layout_wrapped_line<Text>(
        ...
        max_lines: Option<usize>,
    ) -> Arc<WrappedLineLayout> where
        Text: AsRef<str>,
        SharedString: From<Text>,
    {
        self.layout_wrapped_line_spaced(text, font_size, runs, wrap_width, max_lines, px(0.))
    }

    /// KaminIDE patch: `layout_wrapped_line` с трекингом. Трекинг применяется
    /// ТОЛЬКО когда переноса нет (`wrap_width.is_none()`): при переносе
    /// границы считает `compute_wrap_boundaries` по неучтённым ширинам, и
    /// строка порвалась бы не там. Все наши цели трекинга (заголовки секций,
    /// подписи табов, чипы) — однострочные `nowrap` (план 99).
    pub fn layout_wrapped_line_spaced<Text>(
        &self,
        text: Text,
        font_size: Pixels,
        runs: &[FontRun],
    ...
    where
        ...
        SharedString: From<Text>,
    {
        let letter_spacing = if wrap_width.is_some() {
            px(0.)
        } else {
            letter_spacing
        };
        ...
        if let Some((key, layout)) = previous_frame_entry {
            ...
            let unwrapped_layout = self.layout_line_spaced::<&SharedString>(
                &text,
                font_size,
                runs,
                None,
    ...
    where
        ...
        SharedString: From<Text>,
    {
        self.layout_line_spaced(text, font_size, runs, force_width, px(0.))
    }

    /// KaminIDE patch: то же, но с трекингом (`letter-spacing`). Отдельный
    /// метод, а не новый аргумент: `layout_line` зовут 24 раза по вендору,
    /// менять их все ради одного поля — лишний радиус (план 99).
    pub fn layout_line_spaced<Text>(
        &self,
        text: Text,
        font_size: Pixels,
        runs: &[FontRun],
        ...
        SharedString: From<Text>,
    {
        ...
        if let Some(layout) = current_frame.lines.get(key) {
            // KaminIDE patch (#76): счётчики кэша шейпинга.
            crate::frame_perf::CACHED_LINES.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            return layout.clone();
        }
        ...
        if let Some((key, layout)) = self.previous_frame.lock().lines.remove_entry(key) {
            ...
            // KaminIDE patch (#76): промах кэша = реальный шейпинг.
            let shape_started = std::time::Instant::now();
            crate::frame_perf::SHAPED_LINES.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            let text = SharedString::from(text);
            let mut layout = self
                .platform_text_system
                .layout_line(&text, font_size, runs);
            crate::frame_perf::SHAPE_US.fetch_add(
                shape_started.elapsed().as_micros() as u32,
            ...
            // KaminIDE patch: трекинг. CSS добавляет `letter-spacing` ПОСЛЕ
            // каждого символа, включая последний, поэтому ширина растёт на
            // `n × spacing`, а i-й глиф сдвигается на `i × spacing`. Правим
            // ровно там же, где сдвиг под `force_width`, — тогда хит-тест
            // (`index_for_x`) считает по тем же координатам и отдельной
            // правки не требует (план 99)
            if letter_spacing != px(0.) {
                let mut i = 0usize;
                for run in layout.runs.iter_mut() {
                    for glyph in run.glyphs.iter_mut() {
                        glyph.position.x += letter_spacing * (i as f32);
                        i += 1;
                    }
                }
...
struct CacheKey {
    ...
    wrap_width: Option<Pixels>,
    force_width: Option<Pixels>,
    /// KaminIDE patch: трекинг (`letter-spacing`). В ключе ОБЯЗАТЕЛЕН — без
    /// него строка с другим трекингом придёт из кэша со старыми позициями.
    letter_spacing: Pixels,
}
...
struct CacheKeyRef<'a> {
    ...
    wrap_width: Option<Pixels>,
    force_width: Option<Pixels>,
    /// KaminIDE patch: см. `CacheKey::letter_spacing`.
    letter_spacing: Pixels,
}
