...
impl WindowsWindowInner {
    pub(crate) fn handle_msg(
        ...
        let handled = match msg {
            ...
            // KaminIDE patch: не стирать фон — заливка (чёрным) давала полосы
            // по краям при ресайзе blit-окна; кадр рисуется синхронно в
            // WM_SIZE, стирание только мерцает.
    ...
    fn handle_size_msg(&self, handle: HWND, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
        ...
        // KaminIDE patch: кадр рисуем СИНХРОННО внутри WM_SIZE — окно меняет
        // размер уже с готовым содержимым, как нативные приложения. Ожидание
        // WM_PAINT из очереди оставляло растянутый старый кадр на глазах
        // (разворот/восстановление: первый корректный кадр приходил через
        // ~200 мс, стенд серии скриншотов).
        //
        // И БУСТ на 600 мс: этот презент DWM теряет за своей анимацией
        // разворота, а следующие WM_PAINT пропускали отрисовку как «не
        // грязную» — окно больше, контент старый до первого события ввода
        // (замер probe: кадр есть за 0.02 с, на экране его нет).
        self.state.borrow_mut().resize_boost_until =
    ...
    fn handle_paint_msg(&self, handle: HWND) -> Option<isize> {
        // KaminIDE patch: см. resize_boost_until — после смены размера кадры
        // принудительные, пока системная анимация не кончилась.
        let boosted = self
            .state
            .borrow()
            .resize_boost_until
            .is_some_and(|t| std::time::Instant::now() < t);
        self.draw_window(handle, boosted)
    }
