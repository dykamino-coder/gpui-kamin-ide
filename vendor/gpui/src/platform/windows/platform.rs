    ...
    fn run(&self, on_finish_launching: Box<dyn 'static + FnOnce()>) {
        ...
        unsafe {
            while GetMessageW(&mut msg, None, 0, 0).as_bool() {
                // KaminIDE patch: WM_CHAR для child-HWND сторонних вью
                // (WebView2 composition input window). gpui транслирует свои
                // клавиши сам в обработчике; его окна — toplevel, поэтому
                // транслируем только сообщения child-окон — иначе в вебвью
                // доходит WM_KEYDOWN без WM_CHAR и текст не печатается.
                if matches!(
                    msg.message,
                    WM_KEYDOWN | WM_KEYUP | WM_SYSKEYDOWN | WM_SYSKEYUP
                ) && GetParent(msg.hwnd).is_ok()
                {
