...
/*
**
**              Backdrop blur (KaminIDE patch)
**
** Цепочка даунсемплов кадра под областью + финальный композит с маской
** скруглённых углов. Один шейдер обслуживает оба типа проходов: blur_pass=1 —
** даунсемпл (tent-фильтр 9 тапов, билинейная выборка), blur_pass=0 — композит
** в бэкбуфер (та же выборка + SDF-маска углов).
*/

struct BlurQuad {
    Bounds bounds;        // целевой прямоугольник в текущем вьюпорте (device px)
    Bounds content_mask;
    Corners corner_radii;
...
BlurVertexOutput blur_vertex(uint vertex_id: SV_VertexID, uint quad_id: SV_InstanceID) {
    float2 unit_vertex = float2(float(vertex_id & 1u), 0.5 * float(vertex_id & 2u));
    BlurQuad q = blur_quads[quad_id];
    BlurVertexOutput output;
    output.position = to_device_position(unit_vertex, q.bounds);
    output.uv = q.src_origin + unit_vertex * q.src_scale;
    output.quad_id = quad_id;
    output.clip_distance = distance_from_clip_rect(unit_vertex, q.bounds, q.content_mask);
    return output;
}
...
float4 blur_fragment(BlurFragmentInput input): SV_Target {
    ...
    c += t_sprite.Sample(s_sprite, input.uv + float2( t.x, 0.0)) * 0.125;
    c += t_sprite.Sample(s_sprite, input.uv + float2(0.0, -t.y)) * 0.125;
    c += t_sprite.Sample(s_sprite, input.uv + float2(0.0,  t.y)) * 0.125;
    c += t_sprite.Sample(s_sprite, input.uv + float2(-t.x, -t.y)) * 0.0625;
    c += t_sprite.Sample(s_sprite, input.uv + float2( t.x, -t.y)) * 0.0625;
    c += t_sprite.Sample(s_sprite, input.uv + float2(-t.x,  t.y)) * 0.0625;
    c += t_sprite.Sample(s_sprite, input.uv + float2( t.x,  t.y)) * 0.0625;
    if (q.blur_pass < 0.5) {
