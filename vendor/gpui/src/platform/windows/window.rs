...
pub struct WindowsWindowState {
    ...
    /// KaminIDE patch: до этого момента каждый WM_PAINT рисует принудительно.
    /// Презент, сделанный во время системной анимации разворота, DWM теряет,
    /// а следующие WM_PAINT пропускали отрисовку как «не грязную» — окно
    /// оставалось со старым контентом до первого события ввода.
    pub resize_boost_until: Option<std::time::Instant>,
...
impl WindowsWindow {
    pub(crate) fn new(
        ...
        // KaminIDE patch: env перечитывается ПРИ КАЖДОМ создании окна —
        // main-окну нужен blit (WebView2-чайлды поверх), overlay-окну
        // dcomp с premultiplied-альфой (настоящая прозрачность поповеров).
        let disable_direct_composition = std::env::var("GPUI_DISABLE_DIRECT_COMPOSITION")
            .map(|value| value == "true" || value == "1")
            .unwrap_or(disable_direct_composition);
...
impl PlatformWindow for WindowsWindow {
    ...
    fn d3d_device_raw(&self) -> Option<*mut std::ffi::c_void> {
        Some(self.0.state.borrow().renderer.d3d_device_raw())
    }

    fn d3d_context_raw(&self) -> Option<*mut std::ffi::c_void> {
        Some(self.0.state.borrow().renderer.d3d_context_raw())
    }
    ...
    fn register_external_texture(
        ...
        // Указатель приходит уже с AddRef от вызывающего: заворачиваем во
        // владеющий тип, дальше временем жизни распоряжается атлас.
        let texture: windows::Win32::Graphics::Direct3D11::ID3D11Texture2D =
            unsafe { windows::core::Interface::from_raw(texture) };
        self.0
            .state
            .borrow()
            .renderer
    ...
    fn update_external_texture(&self, id: AtlasTextureId, texture: *mut std::ffi::c_void) -> bool {
        if texture.is_null() {
            return false;
        }
        let texture: windows::Win32::Graphics::Direct3D11::ID3D11Texture2D =
        ...
        self.0
            ...
            .borrow()
            .renderer
            .update_external_texture(id, texture)
    }

    fn unregister_external_texture(&self, id: AtlasTextureId) {
        self.0
            .state
            .borrow()
            .renderer
            .unregister_external_texture(id);
    }
...
fn register_window_class(icon_handle: HICON) {
    ...
    ONCE.call_once(|| {
        let wc = WNDCLASSW {
            ...
            // KaminIDE patch: БЕЗ кисти фона — чёрная заливала непокрытую
            // область при ресайзе (blit) чёрными полосами по краям.
            // Стирание выключено и в WM_ERASEBKGND (`events.rs`).
