...
Первый режим не начинает независимый backlog, второй не добавляет следующие
планируемые пачки. Новый child в пределах текущего поручения обрабатывается по
разделу «Задача, обнаруженная мейнтейнером» ниже. Если в начале backlog-run уже
есть открытые PR, agent сначала
