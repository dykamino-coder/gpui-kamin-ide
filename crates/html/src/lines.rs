...
pub fn rules(c: &crate::computed::Computed) -> Option<Wrap> {
    let wrap = wrap_of(c);    let needed = wrap.break_spaces
        ...
        || wrap.anywhere
        || wrap.keep_all
        || c.keep_spaces == Some(true)
        // Выключка по ширине: остаток раздаётся пробелам строки, а готовую
        // строку движка растянуть нечем — слова ставит на места только своя
        // раскладка.
        || c.text_align == Some(crate::computed::TextAlign::Justify)
        || c.text_align_last.is_some()
        // Свисающий знак выходит за край коробки — движок так не умеет.
        || c.hanging.is_some();
    // Своя раскладка считает ВЕСЬ текст: правила переноса, выключка и
    // свисающая пунктуация должны решаться одним алгоритмом, иначе соседние
    // абзацы одной страницы ломаются по-разному. Признак `needed` остаётся
    // для отладки: с ним видно, где без своей раскладки не обойтись вовсе.
    let _ = needed;
    Some(wrap)
}
