...
vi.mock('preact/hooks', () => ({
  ...
  useLayoutEffect: (effect: () => void | (() => void)) => { mocked.cleanup = effect() },
...
describe('useInputDraft', () => {
  it('restores a per-tab textarea draft without any PTY/bridge dependency', () => {
    ...
    const textarea = {
      ...
      dispatchEvent: vi.fn((event: Event) => { listeners.get(event.type)?.(event); return true }),
    } as unknown as HTMLTextAreaElement
    const setHasText = vi.fn()

    useInputDraft({ tabId: 'tab-1', textareaRef: { current: textarea }, setHasText })
    ...
    // The hook accepts storage + DOM only. A future bridge send added during
    // restore would require changing this public boundary and this test setup.
