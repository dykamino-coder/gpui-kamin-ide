...
describe('replaceSkillsOverlay', () => {
  ...
  it('replaces a legacy user symlink without writing project skills through it', () => {
    const root = tempDir()
    const user = path.join(root, 'user')
    const project = path.join(root, 'project')
    const destination = path.join(root, 'session', '.claude', 'skills')
    ...
    fs.symlinkSync(user, destination, 'junction')

    replaceSkillsOverlay(destination, user, project)

    expect(fs.lstatSync(destination).isSymbolicLink()).toBe(false)
    expect(fs.existsSync(path.join(destination, 'project-only/SKILL.md'))).toBe(true)
    expect(fs.existsSync(path.join(user, 'project-only/SKILL.md'))).toBe(false)
  })
})
