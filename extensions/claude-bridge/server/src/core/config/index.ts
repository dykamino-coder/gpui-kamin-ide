...
export const config: Config = {
  port:
    args.port ??
    (process.env.CLAUDE_PROXY_PORT && Number.isFinite(parseInt(process.env.CLAUDE_PROXY_PORT, 10))
      ? parseInt(process.env.CLAUDE_PROXY_PORT, 10)
      : undefined) ??
    opencode.port ??
    DEFAULTS.port,

  host: args.host ?? process.env.CLAUDE_PROXY_HOST ?? opencode.host ?? DEFAULTS.host,

  debug:
    args.debug || process.env.DEBUG === 'true' || process.env.DEBUG === '1' || process.env.CLAUDE_PROXY_DEBUG === '1',

  verbose: args.verbose || process.env.VERBOSE === 'true' || process.env.VERBOSE === '1',
}

const packageManifest = JSON.parse(readFileSync(new URL('../../../package.json', import.meta.url), 'utf-8')) as {
  version?: unknown
}
